...
static bitvec_t getbits2(bitvec_t *B, i64 i, i64 d) // 遅い
{
  bitvec_t x;
...
int f;
  ...
  for (w = 0; w <= NCK*MAXGRP; w++) { // w: 数の合計値
    for (k = 1; k <= MAXGRP; k++) { // k: 要素数
      ...
      for (j=0; j<RR && i+j<n; j+=L) {
        if (pass==2) {
          if (opt & SDARRAY_COMPPTR) {
            ...
            da->ps[(i+j)/L] = bss; // small blockの先頭
          }
        }
        if (opt & SDARRAY_RR) {
          d = getbit(buf,i+j+0);
...
//          printf("grouping i=%ld j=%ld\n",i,j);
          ...
          for (k=0;k<L && i+j+k<n ;k+=D) {
            ...
            if ((k1-k2+1 > MAXGRP) || (w2+f > NCK*MAXGRP)
                || (enumtbl_w[k1-k2+1][w2+f] == NULL)) { // できない
...
//                      blk,num_blk[blk],w_blk[blk]);
          ...
          if (k1-k2 > 0) { // 最後を1グループに
            if ((k1-k2 > MAXGRP) || (w2 > NCK*MAXGRP)
                || (enumtbl_w[k1-k2][w2] == NULL)) { // できない
...
// pointer
  ...
  for (pass = 1; pass <= 2; pass++) {
    ...
    bss = 0;
    for (j=0; j<n; j+=L) {
      if (pass==2) da->ps[(i+j)/L] = bss; // small blockの先頭
